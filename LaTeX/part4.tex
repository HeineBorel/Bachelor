...
This section will closely follow \cite{LesHaupt}. The reader not familiar
...
Departing from this example, let $\hs$ be a Hilbert space.
...
$\cf(X)$, since  \unsure{richtige Formulierung?}the expansion of measure is unique.
...
$(ii) \Rightarrow (i)$: \newline
 ...
 \]
 As shown in $(ii) \Rightarrow (iii)$, the limit of $\Phi_{\tilde{f_n}}$ exists. Hence, for $g \in \hs$
...
% = \int f_n \dm{g,h} 
% = \int f \dm{g, E_{n} h }
% = \int \dm{g, \Phi_f E_{n} h} \\
% &= \gen{g, \Phi_f E_{n} h} 
% = \gen{\Phi_f \str g, E_{n} h}
...
can be easily seen, using the polarization identity, found in 
\cite[Ch. 4.6]{LinOps}.
