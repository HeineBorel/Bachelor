...
and hence of no interest to us. 
 ...
 Let $f \in \Lzerophi$.
 \begin{align*}
 D(f) &\coloneqq \big\lbrace h \in \hs \bigm | f \in \Lone  (\m{g,h})
 \text{ for all } g
 \in \hs\\
...
\begin{proof}
 ...
 \end{align*}
  ...
  \end{align*}
where the last equality holds, because $h \in D(f_m - f_n)$ and thus $f \in \Ltwophi$. 
...
We claim $E_{\mc{A}_n}(\hs) \subset D(f)$, where $\mc{A}_n = 
\setl x \in X \bigm | | f(x) | \leq n \setr$.
...
Let $h \in \hs = D(1)$. By Lemma \ref{maintheorem2} we have
\[
h = \Phi_1 h = \lim_{n \to \infty} \Phi_{1_{\mc{A}_n}} =
\lim_{n \to \infty} E_{\mc{A}_n} h , 
\]
which gives $D(f) \subset \hs$ is dense, as $E_{\mc{A}_n} h \in D(f)$ by
the claim. 

Now we claim $ \Phi_{\overline{f}} \subset \Phi_f \str$.\newline
Let  $g,h \in D(f) = D(\overline{f})$. Using Theorem \ref{measureproperties} $
(iii)$, one has
\begin{align*}
...
Thus $D(\Phi_f \str) = D(\Phi_{\overline{f}})$ and 
...
\begin{proof}
  ...
  \dm{h,h} < \infty  \setr = D(\varphi)   \shortintertext{ and,}
...
will be omitted.
  ...
  \]
  Let $h \in D(\varphi) \cap D(\psi)$. By Lemma \ref{maintheorem2}, 
  \begin{gather*}
  ...
  \end{gather*}
  Thus $h \in D(\varphi+\psi)$. Furthermore, for  $g \in \hs$
  \[
